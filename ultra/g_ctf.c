...
gitem_t	*item_tech1, *item_tech2, *item_tech3, *item_tech4, *item_tech5, *item_tech6;

// Knightmare- added index for CTF pics
int	imageindex_i_ctf1;
int	imageindex_i_ctf2;
int	imageindex_i_ctf3;
int	imageindex_i_ctf3s;
int	imageindex_i_ctf1d;
int	imageindex_i_ctf2d;
int	imageindex_i_ctf3d;
int	imageindex_i_ctf3ds;
int	imageindex_i_ctf1t;
int	imageindex_i_ctf2t;
int	imageindex_i_ctf3t;
int	imageindex_i_ctf3ts;
int	imageindex_i_ctfj;
int	imageindex_sbctf1;
int	imageindex_sbctf2;
int	imageindex_sbctf3;
int	imageindex_ctfsb1;
int	imageindex_ctfsb2;
int	imageindex_ctfsb3;
...
// AJ added tech5
	...
	item_tech6 = FindItemByClassname("item_tech6");
	// ERASER

}

// Knightmare- added CTF Precache function
void CTFPrecache (void)
{
	...
	{
		imageindex_i_ctf1	= gi.imageindex("redlion_i");
		imageindex_i_ctf2	= gi.imageindex("bluewolf_i");
		imageindex_i_ctf1d	= gi.imageindex("redflagdown");
		imageindex_i_ctf2d	= gi.imageindex("blueflagdown");
		imageindex_i_ctf1t	= gi.imageindex("redflaggone");
		imageindex_i_ctf2t	= gi.imageindex("blueflaggone");
		imageindex_i_ctfj	= gi.imageindex("i_ctfj");
		imageindex_sbctf1	= gi.imageindex("sbctf1");
		imageindex_sbctf2	= gi.imageindex("sbctf2");
		imageindex_ctfsb1	= gi.imageindex("redtag");
		imageindex_ctfsb2	= gi.imageindex("bluetag");
	}
	else
	{
	...
	if ((targ->client->pers.inventory[ITEM_INDEX(enemy_flag_item)]) || (targ->client->pers.inventory[ITEM_INDEX(enemy_flag_item2)]))
	{
		...
		for (i = 1; i <= maxclients->value; i++) {
			...
			if (ent->inuse && ent->client->resp.ctf_team == otherteam)
				...
				if (ent->inuse && ent->client->resp.ctf_team == otherteam2)
					ent->client->resp.ctf_lasthurtcarrier = 0;
		}
		return;
	}
...
// end AJ
	...
	// same team, if the flag at base, check to he has the enemy flag
// AJ changed for 3tctf compatibility
		...
		//		 (other->client->pers.inventory[ITEM_INDEX(flag3_item)]))
			...
			/*
				...
				if (other->client->pers.inventory[ITEM_INDEX(flag3_item)])
				{
					...
					// other gets another 15 frag bonus
				//	other->client->resp.score += CTF_CAPTURE_BONUS;
					CTFResetFlag(CTF_TEAM3);
				}*/
				...
				//	my_bprintf(PRINT_HIGH, "%s made a double capture!\n", other->client->pers.netname);
					my_bprintf(PRINT_HIGH, "%s captured the %s and %s flags for a double capture!\n",
				...
				if ( ttctf->value && (ctf_team == CTF_TEAM1) && (captures == 1) )
				{
					...
					{
					//	my_bprintf(PRINT_HIGH, "%s completed a double capture!\n", other->client->pers.netname);
						if (captured_flag_item != NULL)
							my_bprintf(PRINT_HIGH, "%s captured the %s flag for a double capture!\n",
								other->client->pers.netname, CTFTeamName(CTFFlagTeam(captured_flag_item)) );
				...
				for (i = 1; i <= maxclients->value; i++)
				{
					...
					else if (player->client->resp.ctf_team == other->client->resp.ctf_team)
					{
						...
						if (player->client->resp.ctf_lastreturnedflag + CTF_RETURN_FLAG_ASSIST_TIMEOUT > level.time)
						{
							...
							player->client->resp.score += captures * CTF_RETURN_FLAG_ASSIST_BONUS;
						}
						if (player->client->resp.ctf_lastfraggedcarrier + CTF_FRAG_CARRIER_ASSIST_TIMEOUT > level.time)
						{
							...
							player->client->resp.score += captures * CTF_FRAG_CARRIER_ASSIST_BONUS;
						}
					}
				}
				return false;
...
// end AJ
	...
	// ERASER: send this bot back to its base!
	if (other->bot_client)
	{
		gitem_t	*sphere;
		...
		else if (ttctf->value && other->client->resp.ctf_team == CTF_TEAM3)
			other->movetarget = flag3_ent;
		other->movetarget_want = WANT_SHITYEAH;
		// ScarFace- if bot has a sphere in its inventory, activate it
		if (other->client->pers.inventory[ITEM_INDEX(FindItemByClassname("item_sphere_defender"))] )
		{
			sphere = FindItemByClassname("item_sphere_defender");
			sphere->use (other, sphere);
		}
		else if (other->client->pers.inventory[ITEM_INDEX(FindItemByClassname("item_sphere_vengeance"))] )
		{
			sphere = FindItemByClassname("item_sphere_vengeance");
			sphere->use (other, sphere);
		}
		else if (other->client->pers.inventory[ITEM_INDEX(FindItemByClassname("item_sphere_hunter"))] )
		{
			sphere = FindItemByClassname("item_sphere_hunter");
			sphere->use (other, sphere);
		}
	}
...
float	team1_rushbase_time, team2_rushbase_time, team3_rushbase_time;	// used by RUSHBASE command
float	team1_defendbase_time, team2_defendbase_time, team3_defendbase_time;
...
#define CTF_DEBUG	false
	...
	edict_t *enemy_flag, *enemy_flag2;
	float dist;
	int		this_team_count, enemy_team_count, enemy_team_count2;
...
//ERASER: make sure the flag is defended
	if (ent->last_seek_enemy < (level.time - 1))
	{
		...
		if (strcmp(ent->classname, "item_flag_team1") == 0)
		{
			...
			else if (ttctf->value && team3_defendbase_time > 0)
			{
				if (enemy_flag2->solid == SOLID_TRIGGER)
				{
					for (i=0; i<num_players; i++)
					{
						plyr = players[i];

						if (plyr->bot_client && plyr->client->resp.ctf_team == ctf_enemy_team2
							&& !CarryingFlag(plyr))
						{
							plyr->target_ent = enemy_flag2;
							plyr->movetarget = enemy_flag2;
							plyr->movetarget_want = 99;
						}
					}
				}
		...
		else if (ttctf->value)
		{
			...
			else if (team2_defendbase_time > 0)
			{
				if (enemy_flag2->solid == SOLID_TRIGGER)
				{
					for (i=0; i<num_players; i++)
		...
		this_team_count = enemy_team_count = enemy_team_count2 = 0;
		...
		for (i=0; i<num_players; i++)
		{
			...
			else if (ttctf->value && plyr->client->resp.ctf_team == ctf_enemy_team2)
			{
				enemy_team_count2++;
			}
	...
	//					&&	((dist = entdist(plyr, ent)) > BOT_GUARDING_RANGE)
		...
		for (i=0; i<num_players; i++)
		{
			if (	(players[i]->client->resp.ctf_team == ctf_enemy_team)
				|| (ttctf->value && players[i]->client->resp.ctf_team == ctf_enemy_team2)
			...
			if (count < ((int)ceil((1.0*(float)enemy_team_count)/4.0)))
			{
				for (i=0; i<num_players; i++)
				{
					if (!players[i]->bot_client)
						continue;
					if (ttctf->value && players[i]->client->resp.ctf_team != ctf_enemy_team
						&& players[i]->client->resp.ctf_team != ctf_enemy_team2)
						continue;
					else if (players[i]->client->resp.ctf_team != ctf_enemy_team)
						continue;
...
void CTFCalcScores (void)
{
	...
	ctfgame.total1 = ctfgame.total2 = ctfgame.total3 = 0;
...
// AJ - add green team status tile
	if (ttctf->value)
	{
		if (ent->client->resp.ctf_team == CTF_TEAM3)
			p3 = imageindex_i_ctf3s;		// gi.imageindex ("3tctfgs");
		else
			p3 = imageindex_i_ctf3;	// gi.imageindex ("3tctfg");
		...
		if (e != NULL)
		{
			if (e->solid == SOLID_NOT)
			{
				...
				// check if on player, default to dropped
				if (ent->client->resp.ctf_team == CTF_TEAM3)
					p3 = imageindex_i_ctf3ds;	// gi.imageindex ("3tctfgds");
				else
					p3 = imageindex_i_ctf3d;	// gi.imageindex ("3tctfgd");
				for (i = 1; i <= maxclients->value; i++)
					if (g_edicts[i].inuse &&
						g_edicts[i].client->pers.inventory[ITEM_INDEX(flag3_item)])
					{
						// enemy has it
						if (ent->client->resp.ctf_team == CTF_TEAM3)
							p3 = imageindex_i_ctf3ts;	// gi.imageindex ("3tctfgts");
						else
							p3 = imageindex_i_ctf3t;	// gi.imageindex ("3tctfgt");
						break;
					}
			}
				...
				if (ent->client->resp.ctf_team == CTF_TEAM3)
					...
					p3 = imageindex_i_ctf3d;	// gi.imageindex ("3tctfgd"); // must be dropped
			}
		}
...
// end AJ
	...
	if ( (ctfgame.last_flag_capture) && (level.time - ctfgame.last_flag_capture < 5) )
	{
		...
		else if (ttctf->value && ctfgame.last_capture_team == CTF_TEAM3) {
			if (level.framenum & 8)
				ent->client->ps.stats[STAT_CTF_TEAM3_PIC] = p3;
			else
				ent->client->ps.stats[STAT_CTF_TEAM3_PIC] = gi.imageindex ("i_ctfj");
		}
	}
	...
	if (level.framenum & 8)	// make flag icon blink every 0.8s
	{
		if ( (ent->client->resp.ctf_team == CTF_TEAM1) &&
			ent->client->pers.inventory[ITEM_INDEX(flag2_item)] && ent->client->pers.inventory[ITEM_INDEX(flag3_item)] )
		{
			if (level.framenum & 4)	// make flag icon swap every 0.4s
				ent->client->ps.stats[STAT_CTF_FLAG_PIC] = imageindex_i_ctf3;
			else
				ent->client->ps.stats[STAT_CTF_FLAG_PIC] = imageindex_i_ctf2;
		}
		else if ( (ent->client->resp.ctf_team == CTF_TEAM2) &&
			ent->client->pers.inventory[ITEM_INDEX(flag1_item)] && ent->client->pers.inventory[ITEM_INDEX(flag3_item)] )
		{
			if (level.framenum & 4)	// make flag icon swap every 0.4s
				ent->client->ps.stats[STAT_CTF_FLAG_PIC] = imageindex_i_ctf3;
			else
			...
			ent->client->pers.inventory[ITEM_INDEX(flag1_item)] && ent->client->pers.inventory[ITEM_INDEX(flag2_item)] )
		{
			if (level.framenum & 4)	// make flag icon swap every 0.4s
				ent->client->ps.stats[STAT_CTF_FLAG_PIC] = imageindex_i_ctf2;
			else
	...
	// end AJ
		else if ( (ent->client->resp.ctf_team == CTF_TEAM1 || ent->client->resp.ctf_team == CTF_TEAM3) &&
...
// AJ added LMCTF filename support
	...
	{
		if (ent->client->resp.ctf_team == CTF_TEAM1)
			ent->client->ps.stats[STAT_CTF_JOINED_TEAM1_PIC] = imageindex_i_ctfj;	// gi.imageindex ("i_ctfj");
		else if (ent->client->resp.ctf_team == CTF_TEAM2)
			ent->client->ps.stats[STAT_CTF_JOINED_TEAM2_PIC] = imageindex_i_ctfj;	// gi.imageindex ("i_ctfj");
	}
// end AJ
...
// AJ simulated offhand grapple sound
		if (!sk_new_hand_hook->value) //ScarFace- changed
...
void CTFGrapplePull (edict_t *self)
{
	...
	// AJ: changed constant CTF_GRAPPLE_PULL_SPEED to hook_pullspeed->value
		VectorScale(hookdir, sk_hook_pullspeed->value, hookdir);
...
// AJ - make the CTF grapple look like an expert grapple for the bots
	if (sk_new_hand_hook->value && self->bot_client) //ScarFace changed
...
// AJ: changed CTF_GRAPPLE_SPEED to hook_speed->value
	CTFFireGrapple (ent, start, forward, damage, sk_hook_speed->value, effect);
	...
	*string = 0;
	...
	if (ttctf->value)
		Com_sprintf (string, sizeof(string), "if 24 xv -64 yv 8 pic 24 endif "
			...
			"if 29 xv 256 yv 8 pic 29 endif "
		...
		*entry = 0;
		...
		//	sprintf(entry+strlen(entry),
			Com_sprintf (tmp, sizeof(tmp),
				...
				cl->pers.netname);
			Com_strcat (entry, sizeof(entry), tmp);
...
// AJ removed the flag logo from LMCTF
			...
			if (cl_ent->bot_client)
			{
				if (showbotping->value)
				{
					...
					if (cl->ping < 0)
						cl->ping = 0;
					Com_sprintf (ping, sizeof(ping), "%3i", cl->ping);
				}
				else
					Com_strcpy (ping, sizeof(ping), "bot");
			}
			else
				Com_sprintf (ping, sizeof(ping), "%3i", (cl->ping > 999) ? 999 : cl->ping);
			...
			if (ttctf->value)
			{
				if (cl_ent->client->pers.inventory[ITEM_INDEX(flag2_item)] && cl_ent->client->pers.inventory[ITEM_INDEX(flag3_item)]) {
				...
				//	sprintf(entry + strlen(entry), "xv -16 yv %d picn sbfctf3 ",
...
// AJ removed the flag logo from LMCTF
			...
			if (ttctf->value)
			{
				if (cl_ent->client->pers.inventory[ITEM_INDEX(flag1_item)] && cl_ent->client->pers.inventory[ITEM_INDEX(flag3_item)]) {
				...
				//	sprintf (entry + strlen(entry), "xv 144 yv %d picn sbfctf3 ",
		...
		//	sprintf(entry+strlen(entry),
			...
			if (cl_ent->client->pers.inventory[ITEM_INDEX(flag1_item)] && cl_ent->client->pers.inventory[ITEM_INDEX(flag2_item)]) {
			...
			//	sprintf (entry + strlen(entry), "xv 304 yv %d picn sbfctf2 ",
...
// AJ added tech5
		...
		||	(other->client->pers.inventory[ITEM_INDEX(item_tech6)]) )
	{
		CTFHasTech(other);
...
// AJ added for HUD display of current rune/tech
	...
	else if (other->client->pers.inventory[ITEM_INDEX(item_tech6)])
		other->client->ps.stats[STAT_LITHIUM_RUNE] = CS_TECH6;
// end AJ
...
int TechCount (void)
{
	...
	for (i = 1; i < globals.num_edicts; i++, mapent++)
	{
		...
		if ( (!strcmp(mapent->classname, "item_tech1")) || (!strcmp(mapent->classname, "item_tech2"))
			...
			|| (!strcmp(mapent->classname, "item_tech5")) || (!strcmp(mapent->classname, "item_tech6")) )
			count++;
	}
...
void RemoveTechs (int oldrunecount, int newrunecount, int numrunetypes)
{
	...
	while ((ctf_tnames[i]) && (j > newrunecount) && (j > numrunetypes)) //leave at least 1 of each rune
	{
...
void CTFApplyAmmogenSound (edict_t *ent)
{
	...
	{
		ent->client->ctf_techsndtime = level.time + 1;
		gi.sound(ent, CHAN_VOICE, gi.soundindex(rune_ammogen_sound->string), volume, ATTN_NORM, 0);
	}
}
...
//ScarFace- added mission pack sutff
struct {
	char *classname;
	int priority;
} loc_names[] =
...
static void CTFSay_Team_Location (edict_t *who, char *buf, size_t bufSize)
{
	...
	while ((what = G_Find(what, FOFS(classname), hot->classname)) != NULL)
	{
		...
		// ScarFace added- 3Team CTF support
		if (ttctf->value)
		{
				...
				&& (flag3 = G_Find(NULL, FOFS(classname), "item_flag_team3")) != NULL)
			{
				...
				if (hotdist < newdist && hotdist < newdist2)
					nearteam = CTF_TEAM1;
				else if (newdist < hotdist && newdist < newdist2)
					nearteam = CTF_TEAM2;
	...
	if (nearteam == CTF_TEAM1)
		...
		Com_strcat (buf, bufSize, "the green ");
	else
...
static void CTFSay_Team_Enemy (edict_t *who, char *buf, size_t bufSize)
{
	if (who->enemy && who->enemy->client)
		Com_strcpy (buf, bufSize, who->enemy->client->pers.netname);
	else
		Com_strcpy (buf, bufSize, "none");
}
	...
	for (p = outmsg; *msg && (p - outmsg) < sizeof(outmsg) - 1; msg++)
	{
		if (*msg == '%') {
			switch (*++msg) {
				...
				case 'e':
				case 'E':
					CTFSay_Team_Enemy(who, buf, sizeof(buf));
				//	strcpy(p, buf);
				//	p += strlen(buf);
					if (strlen(buf) + (p - outmsg) < sizeof(outmsg) - 1) {	// was -2
						Com_strcpy (p, sizeof(outmsg) - (p - outmsg) - 1, buf);
...
pmenu_t ttctf_joinmenu[] = {
	{ "*Quake II",			PMENU_ALIGN_CENTER, NULL, NULL },
	{ "*3Team CTF",	PMENU_ALIGN_CENTER, NULL, NULL },
	{ NULL,					PMENU_ALIGN_CENTER, NULL, NULL },
	{ NULL,					PMENU_ALIGN_CENTER, NULL, NULL },
	{ "Join Red Team",		PMENU_ALIGN_LEFT, NULL, CTFJoinTeam1 },
	{ NULL,					PMENU_ALIGN_LEFT, NULL, NULL },
...
int TTCTFUpdateJoinMenu (edict_t *ent)
{
	...
	ttctf_joinmenu[4].text = "Join Red Team";
	ttctf_joinmenu[4].SelectFunc = CTFJoinTeam1;
	ttctf_joinmenu[6].text = "Join Blue Team";
	ttctf_joinmenu[6].SelectFunc = CTFJoinTeam2;
	ttctf_joinmenu[8].text = "Join Green Team";
	ttctf_joinmenu[8].SelectFunc = CTFJoinTeam3;
...
void TTCTFOpenJoinMenu (edict_t *ent)
{
	...
	if (ent->client->chase_target)
		...
		team = 8;
	PMenu_Open(ent, ttctf_joinmenu, team, sizeof(ttctf_joinmenu) / sizeof(pmenu_t));
}
