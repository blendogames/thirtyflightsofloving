...
{
	if ((bot_calc_nodes->value) && (ent->movetarget) && (ent->movetarget->last_trail_dropped)
		&& (ent->movetarget->last_trail_dropped->trail_index)
		&& (ent->frags != 1) && plat_ent && (ent->movetarget->groundentity == ent))
	{
		int i;
		vec3_t	save_org;
		...
		if (ent && start_trail) // ScarFace- crash protection
			start_trail->target_ent = ent; // crashes here
...
//PGM
	...
	*/
	...
	//PGM
//	if (self->team)
	if ( self->team && (level.maptype == MAPTYPE_ROGUE) )	// Knightmare- this is for Rogue maps only!
	{
		edict_t *e;
		vec3_t	dir, dst;
...
void SP_func_door_secret (edict_t *ent)
{
	...
	{
		// Knightmare- we can allow user-set healh here
		// now that the correct die function is set
	//	ent->health = 0;
		if (!ent->health)
			ent->health = 1;
