...
void gib_fade (edict_t *self)
{
	if (self->s.effects & EF_GREENGIB) // Remove glow from gekk gibs
		self->s.effects &= ~EF_BLASTER;
	self->s.renderfx = RF_TRANSLUCENT;
	self->nextthink = level.time + 2;
	self->think = gib_fade2;
...
/*	if ((int)mega_gibs->value) // don't get stuck in corners, give bbox
	{
	...
	gib->think = gib_fade;
	if ((int)mega_gibs->value) // overkill gibs stick around longer
		gib->nextthink = level.time + 30 + random()*10;
	else  //normal
...
#if 0	// Knightmare- these are unused
// RAFAEL
...
void func_explosive_explode (edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point)
{
	...
	int		mass;
	edict_t *tempent;
	//ScarFace- for the Rogue trains
	edict_t	*master;
	qboolean	done = false;

	if (self->solid == SOLID_NOT) //ScarFace- prevent explosion triggering from toggling back
		return;
...
void SP_func_explosive (edict_t *self)
{
	...
	if (self->team) //ScarFace- remove func_explosives that are part of a train team
	{
		G_FreeEdict (self);
		return;
	}
...
//	}
	...
	if (self->spawnflags & 4)
		self->s.effects |= EF_ANIM_ALLFAST;
	//ScarFace- func_explosives are destroyable
