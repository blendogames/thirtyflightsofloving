...
//	gi.dprintf("%x killed %x\n", attacker, targ);
	...
	//	gi.dprintf("owner died/suicided. dropping\n");
		...
		targ->s.effects &= ~EF_TAGTRAIL; //ScarFace- remove tag token effect
	//	tag_owner = NULL;
	//	tag_count = 0;
	}
//	else
...
//	gi.dprintf(">>%s picked up tag token at %s\n", other->client->pers.netname, vtos(other->s.origin));
...
//	gi.dprintf(">>Respawning tag token at %s\n", vtos(ent->s.origin));
...
//	tag_token->touch = NULL;
	...
	//ScarFace- add this item to the list
	bonus_head = AddToItemList(tag_token, bonus_head);
...
//	gi.dprintf("Spawning tag token at %s\n", vtos(self->s.origin));
	...
	//ScarFace- it is set to determine when to respawn in droptofloor
	gi.linkentity(self);
	//ScarFace- add this item to the list
	bonus_head = AddToItemList(self, bonus_head);
}
