...
// end AJ
	...
	if (delta > 30)
	{
		if (ent->health > 0)
		{
			...
			else if (sk_hook_style->value==1 && ent->client->hook_on && ent->client->hook)
				return;
			else
				ent->s.event = EV_FALL;
		}
...
//ZOID
	...
	if (ent->client->quadfire_framenum > level.framenum)
	{
		remaining = ent->client->quadfire_framenum - level.framenum;
		if (remaining > 30 || (remaining & 4) )
			ent->s.effects |= EF_DOUBLE | EF_PENT;
	}
...
//frame
		if (client->ctf_grapple || client->hook) {
