...
void sphere_think_explode (edict_t *self)
{
	...
	//ScarFace- if owner is a bot flag carrier, activate another sphere if they have it
	if (self->owner && self->owner->client && self->owner->bot_client && ctf->value && CarryingFlag(self->owner))
	{
		if (self->owner->client->pers.inventory[ITEM_INDEX(FindItemByClassname("item_sphere_defender"))] )
		{
			sphere = FindItemByClassname("item_sphere_defender");
			sphere->use (self->owner, sphere);
		}
		else if (self->owner->client->pers.inventory[ITEM_INDEX(FindItemByClassname("item_sphere_vengeance"))] )
		{
			sphere = FindItemByClassname("item_sphere_vengeance");
			sphere->use (self->owner, sphere);
		}
		else if (self->owner->client->pers.inventory[ITEM_INDEX(FindItemByClassname("item_sphere_hunter"))] )
		{
