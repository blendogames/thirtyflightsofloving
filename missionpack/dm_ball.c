...
void DBall_SelectSpawnPoint (edict_t *ent, vec3_t origin, vec3_t angles, int *style, int *health)
{
	...
	SelectSpawnPoint(ent, origin, angles, &spawn_style, &spawn_health);
...
void DBall_GameInit (void)
{
	...
	gi.cvar_set ("sv_stopspeed", "0");
//	sv_stopspeed->value = 0;
	dball_team1_goalscore = 0;
	dball_team2_goalscore = 0;

	gi.cvar_set ("dmflags",  va("%i", (int)dmflags->value | DF_NO_MINES | DF_NO_NUKES | DF_NO_STACK_DOUBLE | 
