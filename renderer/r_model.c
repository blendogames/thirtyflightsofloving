...
model_t *Mod_ForName (char *name, qboolean crash)
{
	...
	case IDALIASHEADER:
#ifdef MD2_AS_MD3
		loadmodel->extradata = Hunk_Begin (0x500000); // was 0x400000
...
void Mod_LoadFaces (lump_t *l)
{
	...
	// set the drawing flags
		...
		// Quake2Max stuff...
		//if (out->polys)
		if ( out->polys && (out->texinfo->flags & (SURF_TRANS33|SURF_TRANS66|SURF_WARP)) )
		{
			R_CreateVertexLightmap(out);
			R_FindPolyCenters(out);
		}
	}
...
qboolean Mod_ParseWaveFunc (char **data, waveForm_t *out)
{
	...
	if (!Q_strcasecmp(tok, "sin"))
		...
		*out = -1;
		VID_Printf (PRINT_ALL, S_COLOR_YELLOW"Mod_ParseModelScript: unknown waveform '%s'\n", tok);
		return false;
	}
	return true;
}
...
=================
*/
void Mod_ParseModelScript (maliasskin_t *skin, char **data, char *dataStart, int dataSize, char *scriptname)
{
	...
	if (!*data) {
		VID_Printf (PRINT_ALL, S_COLOR_YELLOW"Mod_ParseModelScript: unexpected EOF\n");
		return;
	}
	if (token[0] != '{') {
		VID_Printf (PRINT_ALL, S_COLOR_YELLOW"Mod_ParseModelScript: found %s when expecting {\n", token);
		return;
	}
	...
	{	
		token = COM_Parse (data);
		if (!*data) {
			VID_Printf (PRINT_ALL, "Mod_ParseModelScript: EOF without closing brace\n");
			break;
		}
		...
		else if (!Q_strcasecmp(token, "tcmod"))
		{
			...
			if (!Q_strcasecmp(token, "translate")) {
				...
				{	skinParms->translate_x = 0.0f; break; }
			}
		...
		{	// parse blend parm
			...
			{	// parse 2nd blend parm	
				...
				if (skinParms->blendfunc_src == -1) {
					VID_Printf (PRINT_ALL, S_COLOR_YELLOW"Mod_ParseModelScript: invalid src blend func in %s!\n", scriptname);
					break;
				}

				skinParms->blendfunc_dst = Mod_ParseBlendMode (token2);
				if (skinParms->blendfunc_dst == -1) {
					VID_Printf (PRINT_ALL, S_COLOR_YELLOW"Mod_ParseModelScript: invalid dst blend func in %s!\n", scriptname);
					break;
				}
			}
...
void Mod_LoadModelScript (model_t *mod, maliasmodel_t *aliasmod)
{
	...
	for (i=0; i<aliasmod->num_meshes; i++)
	{
		for (j=0; j<aliasmod->meshes[i].num_skins; j++)
		{
			...
			if (skinname_found)
				Mod_ParseModelScript (&aliasmod->meshes[i].skins[j], &parse_data, buf, buf_size, scriptname);
		}
	}
	FS_FreeFile(buf);
...
void Mod_LoadAliasMD2Model (model_t *mod, void *buffer)
{
	...
	{
		mod->skins[0][i] = R_FindImage ((char *)pheader + pheader->ofs_skins + i*MAX_SKINNAME
			, it_skin);
	}
...
void Mod_LoadAliasMD2ModelNew (model_t *mod, void *buffer)
{
	...
	for (i=0; i < poutmodel->num_frames; i++, pinframe++, poutframe++, poutvert += numVertices)
	{
		...
		for (j=0; j < numIndices; j++)
		{
			poutvert[poutindex[j]].point[0] = (short)pinframe->verts[tempIndex[indRemap[j]]].v[0];
			poutvert[poutindex[j]].point[1] = (short)pinframe->verts[tempIndex[indRemap[j]]].v[1];
			poutvert[poutindex[j]].point[2] = (short)pinframe->verts[tempIndex[indRemap[j]]].v[2];
