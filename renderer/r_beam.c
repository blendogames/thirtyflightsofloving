...
void R_RenderBeam (vec3_t start, vec3_t end, float size, float red, float green, float blue, float alpha)
{
	...
	vec2_t		texCoord[4];
	vec4_t		beamColor;
	int			oldrender=0, rendertype=0, i, baseindex;
	...
	rb_vertex = rb_index = baseindex = 0;
	for (i=0; i<4; i++) {
	...
	indexArray[rb_index++] = baseindex+0;
	indexArray[rb_index++] = baseindex+1;
	indexArray[rb_index++] = baseindex+2;
	indexArray[rb_index++] = baseindex+0;
	indexArray[rb_index++] = baseindex+2;
	indexArray[rb_index++] = baseindex+3;
