...
void R_DrawAliasMD2FrameLerp (dmdl_t *paliashdr, float backlerp)
{
	...
	int			index_xyz;
	float		*lerp;
	int			vertcount, baseindex;
	qboolean	shellModel = currententity->flags & ( RF_SHELL_RED | RF_SHELL_GREEN | RF_SHELL_BLUE | RF_SHELL_DOUBLE | RF_SHELL_HALF_DAM);
...
int		md2shadow_va, md2shadow_index;
/*
=============
...
void R_BuildMD2ShadowVolume (dmdl_t *hdr, vec3_t light, float projectdistance, qboolean nocap)
{
	...
	for (i=0, tris=ot; i<hdr->num_tris; i++, tris++)
	{
		...
		if (currentmodel->edge_tri[i*3+0] < 0 || !trianglefacinglight[currentmodel->edge_tri[i*3+0]])
		{
			...
			indexArray[md2shadow_index++] = baseindex+0;
			indexArray[md2shadow_index++] = baseindex+1;
			indexArray[md2shadow_index++] = baseindex+2;
			indexArray[md2shadow_index++] = baseindex+0;
			indexArray[md2shadow_index++] = baseindex+2;
			indexArray[md2shadow_index++] = baseindex+3;
		}
	...
	if (nocap) {
		//RB_DrawArrays (GL_TRIANGLES, md2shadow_va, md2shadow_index);
		return;
	}
