...
void SV_BeginDownload_f(void)
{
	...
	// if it's in a pk3, cancel download and get name of pk3
	// that file is in, then stuff download command for that file to client
	/*if (file_from_pk3)
	{
		Com_DPrintf ("Aborting download from inside pk3: %s to %s\n", name, sv_client->name);
		if (sv_client->download)
		{
			FS_FreeFile (sv_client->download);
			sv_client->download = NULL;
		...
		MSG_WriteByte (&sv_client->netchan.message, svc_stufftext);
		MSG_WriteString (&sv_client->netchan.message, va("download %s\n", file_from_pk3_name) );
		return;
	}*/
...
void SV_Nextserver (void)
{
	...
	if (!v[0])
	{
		//Com_Printf ("SV_Nextserver: Null nextserver!\n");
		Cbuf_AddText ("killserver\n");
	}
	else
	{
		//Com_Printf ("SV_Nextserver: Changing to nextserver.\n");
		Cbuf_AddText (v);
		Cbuf_AddText ("\n");
	}
	Cvar_Set ("nextserver","");
